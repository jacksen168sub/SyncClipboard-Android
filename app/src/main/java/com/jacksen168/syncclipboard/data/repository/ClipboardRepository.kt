...
class ClipboardRepository(
            ...
            .onEach { config ->
                if (config.url.isNotEmpty()) {
                    try {
                        ...
                        Log.d("ClipboardRepository", "成功初始化API服务: ${config.url}")
                    } catch (e: Exception) {
    ...
    suspend fun testConnection(): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            ...
            settingsRepository.updateConnectionStatus(isConnected)
            
            if (isConnected) {
                Result.success(true)
            } else {
                Result.failure(Exception("连接测试失败: HTTP ${response.code()}"))
            }
        } catch (e: Exception) {
